...
const convertCurrency = useCallback(
  (value) => {
    let position = selectedCurrency?.symbol_position || settingObj?.general?.default_currency?.symbol_position || "before_price";
    let symbol = selectedCurrency?.symbol || settingObj?.general?.default_currency?.symbol || "$";
    let amount = Number(value);
    let exchangeRate = selectedCurrency?.exchange_rate || settingObj?.general?.default_currency?.exchange_rate || 1;
    amount *= exchangeRate;
